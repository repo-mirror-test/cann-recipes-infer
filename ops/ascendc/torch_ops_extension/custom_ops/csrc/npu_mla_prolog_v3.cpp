...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor> npu_mla_prolog_v3(
    ...
    double rmsnorm_epsilon_cq, double rmsnorm_epsilon_ckv, c10::string_view cache_mode, int64_t query_norm_flag, int64_t weight_quant_mode, int64_t kv_quant_mode,
    int64_t query_quant_mode, int64_t ckvkr_repo_mode, int64_t quant_scale_repo_mode, int64_t tile_size, double k_nope_clip_alpha, double qc_qr_scale, double kc_scale)
{
    ...
    if (token_x_dim == 3) {
        if (weight_quant_mode == 2 && kv_quant_mode == 1) {
            // kvcache量化
    ...
    EXEC_NPU_CMD_V1(aclnnMlaPrologV3, token_x, weight_dq, weight_uq_qr, weight_uk, weight_dkv_kr, rmsnorm_gamma_cq,
        ...
        rmsnorm_epsilon_cq, rmsnorm_epsilon_ckv, cache_mode_ptr, query_norm_flag, weight_quant_mode, kv_quant_mode,
        query_quant_mode, ckvkr_repo_mode, quant_scale_repo_mode, tile_size, k_nope_clip_alpha, qc_qr_scale, kc_scale,
...
std::tuple<at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor, at::Tensor> npu_mla_prolog_v3_functional(
    ...
    int64_t query_quant_mode, int64_t ckvkr_repo_mode, int64_t quant_scale_repo_mode, int64_t tile_size, double k_nope_clip_alpha, double qc_qr_scale, double kc_scale)
{
    // TODO，每个新增属性，对输出tensor size的影响，需要梳理和适配
