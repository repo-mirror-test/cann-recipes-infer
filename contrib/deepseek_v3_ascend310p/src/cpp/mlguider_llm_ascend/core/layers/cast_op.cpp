...
void AclnnCastFP16toFP32(aclrtStream& stream, void* a, void* b, int m, int n)
{
    ...
    auto ret = aclnnCastGetWorkspaceSize(
        aclaten, bType, aclbten, &workspaceSize, &handle);
    ...
    if (ret != ACL_SUCCESS)
    {
        ...
        throw std::runtime_error("Get aclnnCastGetWorkspaceSize F16toF32 failed");
    }
    ...
    ret = aclnnCast(workspaceAddr, workspaceSize, handle, stream);
...
void AclnnCastFP32toFP16(aclrtStream& stream, void* a, void* b, int m, int n)
{
    std::vector<int64_t> ashape = {m, n};
    std::vector<int64_t> bshape = {m, n};
    ...
    if (ret != ACL_SUCCESS)
    {
        ...
        throw std::runtime_error("Get aclnnCastGetWorkspaceSize F32toF16 failed");
    }
