...
void Embedding(aclrtStream& stream, void* input, void* weight, void* output, int m, int n, int v)
{
    std::vector<int64_t> ashape = {m};
    std::vector<int64_t> bshape = {v, n};
    ...
    auto ret = aclnnEmbeddingGetWorkspaceSize(aclbten, aclaten, aclcten, &workspaceSize, &handle);
    ...
    if (ret != ACL_SUCCESS)
    {
        ...
        throw std::runtime_error("Get aclnnEmbeddingGetWorkspaceSize failed");
    }
    ...
    ret = aclnnEmbedding(workspaceAddr, workspaceSize, handle, stream);
    ...
    if (ret != ACL_SUCCESS)
    {
        ...
        (void) aclrtDestroyStream(stream);
        throw std::runtime_error("aclnnEmbedding failed");
    }
